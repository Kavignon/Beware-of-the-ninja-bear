	...
	SubShader {
		...
		Pass {
			...
			float4 frag(vertexOutput input) : COLOR
			{
				...
				float4 mainTexColor = tex2D(_MainTex, float2(input.texCoords.xy));
				...
				float3 fragmentColor = float3(_UnlitDarkening.xyz)*float3(mainTexColor.xyz);;
				
				// diffuse illum
				if(attenuation * max(0.0, dot(float3(normalDirection.xyz), lightDirection)) >= _DiffuseThreshold)
				{
					fragmentColor = float3(_LightColor0.xyz)*float3(mainTexColor.xyz);
				}
				...
				{
					fragmentColor = float3(_LightColor0.xyz)*float3(_OutlineColor.xyz);
				}	
