...
class FunctionMacroExpander(object):
	...
	def __StripBrackets__(self, line):
		if self.__toStripOpenBracket:
			openBracket = SpecialChars['FunctionBrackets']['Open']
			firstOpenBracketCol = line.find(openBracket)
			if firstOpenBracketCol != -1:
				line = line.replace(openBracket, SpecialChars['WhiteSpace'], 1)

		if self.__toStripCloseBracket:
			closeBracket = SpecialChars['FunctionBrackets']['Close']
			firstCloseBracketCol = line.find(closeBracket)
			if firstCloseBracketCol != -1:
				line = line.replace(closeBracket, SpecialChars['WhiteSpace'], 1)

		return line

class MemberListExpander(object):
	s_NotFound = None
	def __init__(self, inputLines, lineRegEx, linesToInsert, brackets, separater=SpecialChars['Comma']):
		self.__lines = inputLines
		self.__LineRegEx = lineRegEx
	...
	def __IsClosureLine__(self, line):
		...
		'''
		foundCol = line.find(self.__BlockBrackets['Close']+SpecialChars['StatementEnd'])
		hasClosure = foundCol != -1

		commentCol = line.find(SpecialChars['CppComment'])
...
class ApiHeaderBlobber(object):
	...
	def Export(self, externalLines = []):
		...
		ExportFile(self.OutputHeader, outputLines)
...
class PlaylistExtractor(object):
	...
	def GenerateNewHeaders(self):
		lines = ImportFile(self.inputHeader)
	...
	def ReplaceFunctionMacros(self, inputLines):
		...
		macroDict = \
		{
			'AK_EXTERNAPIFUNC': 'extern'+SpecialChars['WhiteSpace'],
			SpecialChars['Comma']: SpecialChars['WhiteSpace']
		}

		inputPattern = 'AkForceInline'
		outputPattern = SpecialChars['Null']
...
class SimpleStructExtractor(object):
	...
	def ExtractStructAsOneString(self):
		...
		if foundStruct:
			outputString = match.string[match.start() : match.end()]+SpecialChars['PosixLineEnd']
			structLines = outputString.split(SpecialChars['PosixLineEnd'])
...
class PlatformHandler(object):
	...
	def __init__(self, pathMan):
		...
		self.logger = CreateLogger(self.pathMan.Paths['Log'], __file__, self.__class__.__name__)
	...
	def __CovertStructStringToLines(self, structString):
		structLines = structString.split(SpecialChars['PosixLineEnd'])
		for ss in range(len(structLines)):
			structLines[ss] += SpecialChars['PosixLineEnd']
		return structLines
...
def main(pathMan=None):
	...
	ProjectDir = pathMan.Paths['Src_Platform']
	...
	macroDict = \
	{
		# 'AK_EXTERNAPIFUNC': 'extern'+SpecialChars['WhiteSpace']+'__declspec dllexport'+SpecialChars['WhiteSpace'],
		'AK_EXTERNAPIFUNC': 'extern'+SpecialChars['WhiteSpace'],
		SpecialChars['Comma']: SpecialChars['WhiteSpace']+'__cdecl'+SpecialChars['WhiteSpace']
	}
	...
	lineRegEx = '(AK_CALLBACK)([ \t]*\()([ \t]*[_\w \t]+)(,[ \t]*)([_\w]+)([ \t]*\))'
	macroDict = \
	{
		'AK_CALLBACK': 'typedef'+SpecialChars['WhiteSpace'],
		SpecialChars['Comma']: '( __cdecl *'
	}
	...
	memberInserter = MemberListExpander(headerBlobber.lines, lineRegEx, linesToInsert, SpecialChars['CurlyBrackets'])
