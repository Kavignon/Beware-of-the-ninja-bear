...
# //
# //////////////////////////////////////////////////////////////////////
import sys, os, os.path, argparse, subprocess, logging, platform, json
from os import pardir
from os.path import dirname, abspath, join, exists, basename, normpath
...
class AndroidMacBuilder(MultiArchBuilder):
    ...
    def _CreateCommands(self, arch=None, config='Profile'):
        cmd = [self.compiler, '{}'.format(self.solution), arch, config]
        return [cmd]
...
class MacBuilder(PlatformBuilder):
    ...
    def _CreateCommands(self, arch=None, config='Profile'):
        cmd = [self.compiler, '-project', '{}'.format(self.solution), '-configuration', config, 'clean', 'build']
        return [cmd]
...
class WwiseUnityBuilder(object):
    ...
    def _Report(self, results):
        ...
        msg = 'Failed Builds: {}'.format(', '.join(reports['Fail']))
        self.logger.critical(msg)
        msg = 'Succeeded Builds: {}'.format(', '.join(reports['Success']))
        self.logger.critical(msg)
...
def ParseAndValidateArgs(argv=None):
    ...
    parser.add_argument('-u', '--updatepref', action='store_true', dest='isUpdatePref', default=False, help='Flag to set whether or not to overwrite the relevant fields in the preference file with the user specified command options and arguments.')
    ...
    DefaultMsg = 'Use -h for help. Abortted'
...
def ValidateLocationVar(varName, inVarValue, entityName, cliSwitch):
    ...
    Undefined = 'Undefined'
    androidNdkDir = Undefined
    isAndroidNdkSpecifiedInCmd = inVarValue != Undefined
    if isAndroidNdkSpecifiedInCmd:
        # Assume input value is the argparse vector arg.
        androidNdkDir = inVarValue[0]
    else:
        prefAndroidNdkDir = BuildUtil.ReadPreferenceField(varName)
        if not prefAndroidNdkDir is None:
            logger.warn('No {} specified ({}). Fallback to Android NDK folder stored in preference ({}): {}'.format(entityName, cliSwitch, BuildUtil.DefaultPrefFile, prefAndroidNdkDir))
            androidNdkDir = prefAndroidNdkDir
        else:
            if varName in os.environ:
                logger.warn('No {} specified ({}) and no preference found. Fallback to use environment variable: {} = {}'.format(entityName, cliSwitch, varName, os.environ[varName]))
                androidNdkDir = os.environ[varName]
            else:
                logger.error('Undefined environment variable: {}. {}.'.format(varName, DefaultMsg))
                return [None, isAndroidNdkSpecifiedInCmd]
    if not exists(androidNdkDir):
        logger.error('Failed to find {}: {}. {}.'.format(entityName, androidNdkDir, DefaultMsg))
        return [None, isAndroidNdkSpecifiedInCmd]

    return [androidNdkDir, isAndroidNdkSpecifiedInCmd]
