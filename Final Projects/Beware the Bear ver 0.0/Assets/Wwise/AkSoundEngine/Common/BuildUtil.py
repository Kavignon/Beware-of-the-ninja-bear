...
# //
# //////////////////////////////////////////////////////////////////////
import sys, os, os.path, shutil, fnmatch, subprocess, copy, re, logging, logging.handlers, argparse, inspect, traceback, platform, json
...
SupportedPlatforms = \
{
	'Windows': ['Windows', 'Android', 'Metro', 'PS3', 'XBox360'],
...
PlatformSwitches = \
{
	'Windows': '#if UNITY_STANDALONE_WIN || (UNITY_EDITOR && UNITY_STANDALONE_WIN) || (UNITY_EDITOR && UNITY_XBOX360) || (UNITY_EDITOR && UNITY_PS3) || (UNITY_EDITOR && UNITY_ANDROID) || (UNITY_EDITOR && UNITY_METRO)',
	'Mac': '#if UNITY_STANDALONE_OSX || (UNITY_EDITOR && UNITY_STANDALONE_OSX) || (UNITY_EDITOR && UNITY_IOS) || (UNITY_EDITOR && UNITY_ANDROID)',
...
MultiArchSwitches = \
{
	'Android': {'armeabi': 'AK_ARCH_ANDROID_ARMEABI', 'armeabi-v7a': 'AK_ARCH_ANDROID_ARMEABIV7A'},
...
Messages = \
	{
		...
		'CheckLogs': 'Check under the Logs folder for complete logs.'
	}
...
class PathManager(object):
	def __init__(self, PlatformName, Arch=None, unityProjectDir=None):
		...
		#
		self.Paths['UNITY_PROJECT_ROOT'] = unityProjectDir
		self.isDeployToUnityProject = self.Paths['UNITY_PROJECT_ROOT'] != None
		if self.isDeployToUnityProject and not os.path.exists(self.Paths['UNITY_PROJECT_ROOT']):
			self.logger.warn('Failed to find target Unity project folder: {}. Ignored.'.format(self.Paths['UNITY_PROJECT_ROOT']))

		sdkRootDir = None
		isWwiseRootDirDefined = WwiseSdkEnvVar in os.environ
		if not isWwiseRootDirDefined:
		...
		sdkRootDir = os.environ[WwiseSdkEnvVar]
		isUseCygwinStylePath = sys.platform == 'cygwin'
		if isUseCygwinStylePath:
			msg = 'Running under Cygwin Python. This would confuse SWIG. Try agian using system Python instead. Aborted.'
			self.logger.error(msg)
			raise RuntimeError(msg)
		if not (isWwiseRootDirDefined and os.path.exists(sdkRootDir)):
			msg = Messages['Error_NoSdkDir']
			self.logger.error(msg)
			raise RuntimeError(msg)

		self.Paths['Wwise_SDK'] = sdkRootDir
...
def Init(argStr=None):
	...
	pathMan = PathManager(targetPlatform, targetArch)
